...
```json
{
  "query": "Identify research gaps and limitations across these papers.",
  "top_k_per_query": 8
}
```

The frontend hard-codes a predefined `query` per button:
...
The `analysis` string follows the citation rule `[Title (Year), §section_title]` enforced in the synthesis prompt.
...
Accepts the full conversation, asks the LLM to classify the user's intent, then dispatches to one of the three retrieval pipelines or to a `text_response()` fallback (LLM-only, no DB hit).

```json
{
  "messages": [
    {"role": "user", "content": "What are the limitations of recent transformer interpretability work?"}
  ],
  "top_k_per_query": 8
}
```
...
{
  "pipeline": "gaps",
  "query": "What are the limitations of recent transformer interpretability work?",
  "expanded_queries": ["...", "...", "..."],
  "papers": [ /* AnalyzePaper[] — same shape as /analyze/* */ ],
  "analysis": "..."
}
```
